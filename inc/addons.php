 ...
 */
function mercia_amp_menu_dropdown_toggles( $item_output, $item, $depth, $args ) {
	...
	*/
	...
	// Add SVG icon.
	$dropdown_button .= $expanded ? mercia_get_svg( 'collapse' ) : mercia_get_svg( 'expand' );
	...
	$dropdown_button .= sprintf(
		...
		esc_attr( sprintf( "$expanded_state_id ? %s : %s", wp_json_encode( esc_html__( 'Collapse child menu', 'mercia' ) ), wp_json_encode( esc_html__( 'Expand child menu', 'mercia' ) ) ) ),
		esc_html( $expanded ? esc_html__( 'Collapse child menu', 'mercia' ) : esc_html__( 'Expand child menu', 'mercia' ) )
	);
...
add_filter( 'walker_nav_menu_start_el', 'mercia_amp_menu_dropdown_toggles', 10, 4 );
